...
`;
    class CanvaRenderManager {
        ...
        async storeExistingUint32FromGPUBuffer(gpuBuffer, width, height, device) {
            ...
            for (let y = 0; y < height; ++y) {
                result2D.push(u32.slice(y * width, (y + 1) * width));
            }
            window.lastDrawnUint32Pixels = result2D;

            readBuffer.unmap();
            readBuffer.destroy();
        }
        async draw(buffer, height, width) {
            ...
            await this.storeExistingUint32FromGPUBuffer(buffer, 512, 512, this.device);
